<template>
  <jet-dialog-modal :show="createAssayDialog" @close="createAssayDialog = false">
    <template #title> New Assay </template>
      ...
      <div class="relative z-0 mt-1 rounded-lg cursor-pointer">
        <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
          ...
          <div class="sm:col-span-6">
              ...
              <TabPanels class="mt-2">
                <TabPanel class="p-0.5 -m-0.5 rounded-lg">
                  ...
                  <div>
                    <textarea
                      id="description"
                      v-model="createAssayForm.description"
                      name="description"
                ...
                <TabPanel class="p-0.5 -m-0.5 rounded-lg">
                  <div class="border-b">
                    <div class="mx-px mt-px px-3 pt-2 pb-12">
                      <span
                        v-if="createAssayForm.description == ''"
                      ...
                      <span v-else>
                        <div
                          class="prose"
                          v-html="md(createAssayForm.description)"
                        ></div>
                      </span>
                    </div>
                  </div>
            ...
            <jet-input-error
              :message="createAssayForm.errors.description"
              class="mt-2"
            />
          </div>
          <div class="sm:col-span-6">
            <SwitchGroup as="div" class="flex items-center">
              <Switch
                v-model="createAssayForm.is_public"
                :class="[
                  createAssayForm.is_public ? 'bg-green-600' : 'bg-gray-200',
                  'relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500',
                ]"
              >
                <span
                  :class="[
                    createAssayForm.is_public ? 'translate-x-5' : 'translate-x-0',
                    'pointer-events-none relative inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200',
                  ]"
                >
                  <span
                    :class="[
                      createAssayForm.is_public
              ...
              <span class="flex-grow flex ml-4 flex-col">
                <span v-if="!createAssayForm.is_public">
                  <SwitchLabel
                    as="span"
                  ...
                  <SwitchDescription
                    ...
                    >Visibility of the Assay only, not the underlying
    ...
    <template #footer>
      <jet-secondary-button @click="createAssayDialog = false">
        Cancel
      </jet-secondary-button>

      <jet-button
        class="ml-2"
        :class="{ 'opacity-25': createAssayForm.processing }"
        :disabled="createAssayForm.processing"
        @click="createAssay"
      >
        Save
      </jet-button>
...
export default {
  ...
  data() {
    return {
      createAssayForm: this.$inertia.form({
        _method: "POST",
        name: "",
        description: "",
        error_message: null,
      ...
      createAssayDialog: false,
    };
  },

  methods: {
    createAssay() {
      this.createAssayForm.owner_id = this.$page.props.user.id;
      this.createAssayForm.team_id = this.$page.props.user.current_team.id;
      this.createAssayForm.post(route("assays.create"), {
        preserveScroll: true,
        onSuccess: () => {
          this.createAssayDialog = false;
          this.createAssayForm.reset();
        },
        onError: (err) => console.error(err),
      });
    },
    toggleCreateAssayDialog(){
        this.createAssayDialog = !this.createAssayDialog;
    }
  },
};
</script>
