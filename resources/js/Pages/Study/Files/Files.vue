<template>
  <div>
    <study-index :project="project" :study="study" current="Files">
  ...
  selectedFiles.value = checked
    ...
    .map(f => {
      return {
        id: f.id,
        name: f.name,
        updated_at: new Date(Date.parse(f.updated_at)).toLocaleString('de-DE', {
          timeZone: 'Europe/Berlin',
        }),
        path: f.path,
        type: f.type,
        created_at: new Date(Date.parse(f.created_at)).toLocaleDateString(
          'de-DE',
          { timeZone: 'Europe/Berlin' },
        ),
        details: {},
